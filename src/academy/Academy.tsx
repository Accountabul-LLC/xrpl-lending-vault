    ...
    <div className="academy-shell min-w-0 lg:h-[calc(100vh-3.25rem)] lg:min-h-[640px] grid grid-cols-1 lg:grid-cols-[minmax(200px,240px)_minmax(0,1fr)] gap-3 lg:gap-4">
      ...
      <div className="min-w-0 flex flex-col gap-2.5 lg:overflow-auto">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_minmax(220px,280px)] gap-3 min-h-0">
          ...
          <AdvancedRolesPanel onOpenInstitutional={() => changeTrack('institutional')} />
        </div>
...
function AdvancedRolesPanel({ onOpenInstitutional }: { onOpenInstitutional: () => void }) {
  const sim = useSimulation()
  ...
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-950/70 p-3 flex flex-col gap-2 h-fit xl:min-h-[200px]">
      ...
      <p className="text-[11px] text-slate-500">
        ...
        network. For the full desk — originator, underwriter, vault owner vs administrator,
        first-loss, servicing, and credentials — open{' '}
        <button
          type="button"
